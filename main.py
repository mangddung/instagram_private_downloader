...
FOLLOWERS_DIR = os.path.join(OUTPUT_DIR, 'followers')
HIGHLIGHT_MEDIA_DIR = os.path.join(OUTPUT_DIR, 'highlight_media')

os.makedirs(FOLLOWERS_DIR, exist_ok=True)
os.makedirs(HIGHLIGHT_MEDIA_DIR, exist_ok=True)
...
# ds_user_id 입력 필드 제거
# tk.Label(auth_frame, text="ds_user_id:").grid(row=3, column=0, sticky="e", pady=2)
# dsuserid_entry = tk.Entry(auth_frame, width=40)
# dsuserid_entry.grid(row=3, column=1, padx=5, pady=2, sticky="ew")
...
def extract_all_followers_wrapper():
    ...
    try:
        ds_user_id_parsed = sessionid.split('%')[0] 
    ...
    headers = {
        ...
        "Cookie": f"sessionid={sessionid}; csrftoken={csrftoken}; ds_user_id={ds_user_id_parsed};", # 파싱된 ds_user_id 사용
    ...
    extract_all_followers(username, global_user_id, global_total_followers_count, follower_query_hash, headers)
...
def fetch_instagram_data():
    ...
    # .env 파일에서 변수 불러오기
    username_env = os.getenv("INSTAGRAM_USERNAME") # 조회 대상 닉네임 (환경 변수)
    sessionid = os.getenv("INSTAGRAM_SESSIONID")
    csrftoken = os.getenv("INSTAGRAM_CSRFTOKEN")
    # ds_user_id 제거
    ...
    if not (username_env and sessionid and csrftoken and 
            follower_query_hash and highlight_query_hash and highlight_media_query_hash and ds_user_id_parsed):
        root.after(0, messagebox.showerror, "오류", ".env 파일에서 모든 필수 환경 변수를 불러오지 못했습니다. config.env 파일을 확인하세요.")
        return

    # 불러온 값들을 엔트리 필드에 미리 채워넣습니다 (선택 사항, 사용자에게 보여주기 위함)
    root.after(0, username_entry.delete, 0, tk.END)
    root.after(0, username_entry.insert, 0, username_env)
    root.after(0, sessionid_entry.delete, 0, tk.END)
    root.after(0, sessionid_entry.insert, 0, sessionid)
    root.after(0, csrftoken_entry.delete, 0, tk.END)
    root.after(0, csrftoken_entry.insert, 0, csrftoken)
    ...
    try:
        # 1. 유저 ID 조회 (username_env 사용)
        search_url = f"https://www.instagram.com/web/search/topsearch/?query={username_env}"
        root.after(0, update_gui_log, f" - 사용자 검색 요청 중: {username_env}\n")
        res = requests.get(search_url, headers=headers)
        res.raise_for_status()

        user_data = res.json()
        # root.after(0, update_gui_log, f"Search API 응답: {json.dumps(user_data, indent=2)}\n") # JSON 로그 제거
        ...
        try:
            ...
            # root.after(0, update_gui_log, f"총 팔로워 수 API 응답: {json.dumps(follower_count_data, indent=2)}\n") # JSON 로그 제거
        ...
        try:
            ...
            # root.after(0, update_gui_log, f"하이라이트 GraphQL API 응답: {json.dumps(highlight_data, indent=2)}\n") # JSON 로그 제거
            ...
            if not highlight_data or "data" not in highlight_data or "user" not in highlight_data["data"]:
                ...
                if highlight_edges:
                    ...
                    for i, edge in enumerate(highlight_edges):
                        ...
                        root.after(0, lambda h_id=highlight_id, h_title=highlight_title: 
                                add_highlight_row_to_gui(h_id, h_title, user_id, sessionid, csrftoken, ds_user_id_parsed, highlight_media_query_hash, headers, i)) # ds_user_id_parsed 전달
                else:
...
def add_highlight_row_to_gui(highlight_id, highlight_title, user_id, sessionid, csrftoken, ds_user_id, highlight_media_query_hash, headers, idx):
    ...
    tk.Label(highlight_row, text=f"        {idx+1}. ID: {highlight_id}, 제목: {highlight_title}", justify="left", anchor="w").pack(side=tk.LEFT, fill=tk.X, expand=True)
...
def extract_all_followers(username_env, target_user_id, total_followers_count_param, follower_query_hash, headers):
    ...
    while has_next_page:
        ...
        try:
            ...
            # root.after(0, update_gui_log, f"팔로워 페이지 {page_num} API 응답: {json.dumps(data, indent=2)}\n") # JSON 로그 제거
    ...
    # 저장 경로 설정
    username_for_dir = os.getenv("INSTAGRAM_USERNAME")
    username_dir_path = os.path.join(OUTPUT_DIR, username_for_dir)
    os.makedirs(username_dir_path, exist_ok=True)
    followers_filename = os.path.join(username_dir_path, f"{username_env}_followers.txt")
    try:
...
def load_env_vars_into_gui():
    ...
    sessionid = os.getenv("INSTAGRAM_SESSIONID")
    csrftoken = os.getenv("INSTAGRAM_CSRFTOKEN")
    # ds_user_id는 더 이상 env에서 직접 로드하지 않음
    ...
    # dsuserid_entry는 제거되었으므로 해당 코드도 제거
    # if ds_user_id:
    #     dsuserid_entry.insert(0, ds_user_id)
